...
const LoginPage: React.FC = () => {
  ...
  // Check SSO status and read sso_error from URL query params
  useEffect(() => {
    const errMsg = searchParams.get('sso_error');
    if (errMsg) setSsoError(decodeURIComponent(errMsg));

    fetch(`${process.env.REACT_APP_API_URL || ''}/api/auth/sso/status`)
      .then(r => r.json())
          ...
          <div className="bg-surface rounded-2xl shadow-2xl border border-border p-8">
            ...
            {ssoError && (
              <div className="mb-4 px-4 py-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                ...
                <p className="text-xs text-red-700">{ssoError}</p>
              </div>
            )}
            ...
            {ssoEnabled && (
              <div className="mb-6">
                <a
                  href={`${process.env.REACT_APP_API_URL || ''}/api/auth/sso/login`}
                  className="flex items-center justify-center gap-3 w-full py-3 px-4 bg-white border border-border rounded-lg text-sm font-medium text-textPrimary hover:bg-gray-50 hover:border-indigo-300 transition-all duration-150 shadow-sm"
                >
                ...
                <div className="flex items-center gap-3 mt-5 mb-1">
                  ...
                  <span className="text-xs text-textTertiary">or sign in with email</span>
                  <div className="flex-1 border-t border-border" />
                </div>
              </div>
            )}
