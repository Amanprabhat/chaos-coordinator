              ...
              {requests.map(req => {
                ...
                return (
                  <div key={req.id} className={`bg-white rounded-2xl border shadow-sm p-5 transition-all ${
                    ...
                    {isCRType && clientStatusKey === 'pending' && (() => {
                      const STAGE_STEPS: Record<string, { step: number; label: string }> = {
                        csm_review:   { step: 1, label: 'Internal Review' },
                        pm_review:    { step: 2, label: 'Planning & Estimation' },
                        sales_review: { step: 3, label: 'Pricing Review' },
                        admin_review: { step: 4, label: 'Final Approval' },
                      };
                      const stage = req.approval_stage ? STAGE_STEPS[req.approval_stage as string] : undefined;
                      if (!stage) return null;
                      const total = 4;
                      return (
                        <div className="mb-3">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[10px] font-semibold text-indigo-600">Step {stage.step} of {total}: {stage.label}</span>
                            <span className="text-[10px] text-gray-400">{Math.round((stage.step / total) * 100)}% complete</span>
                          </div>
                          <div className="flex gap-1">
                            {Array.from({ length: total }).map((_, i) => (
                              <div key={i} className={`h-1 flex-1 rounded-full ${i < stage.step ? 'bg-indigo-400' : 'bg-gray-100'}`} />
                            ))}
                          </div>
                        </div>
                      );
