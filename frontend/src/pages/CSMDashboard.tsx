...
const CSMDashboard: React.FC = () => {
  ...
  // PM CR review state (for users with role=PM using this dashboard)
  const [pmCrList, setPmCrList]         = useState<any[]>([]);
  const [pmCrModal, setPmCrModal]       = useState<any | null>(null);
  const [pmCrForm, setPmCrForm]         = useState({ pm_notes: '', effort_man_days: '', effort_hours: '' });
  const [pmCrSubmitting, setPmCrSubmitting] = useState(false);
    ...
    (async () => {
      try {
        const [projRes, taskRes, msRes] = await Promise.all([
          fetch(`${process.env.REACT_APP_API_URL || ""}/api/projects`, { signal }),
          fetch(`${process.env.REACT_APP_API_URL || ""}/api/tasks?owner_id=${user.id}`, { signal }),
          fetch(`${process.env.REACT_APP_API_URL || ""}/api/milestones`, { signal }),
        ]);
  ...
  // Stable reference — only recomputes when projects array or user id changes
  const myProjects = useMemo(() => projects.filter(p => {
    ...
    return true;
  }), [projects, isCSM, isPM, user?.id]);

  // Fetch CR requests needing CSM review
  const fetchCRRequests = useCallback(async () => {
    ...
    try {
      ...
      // Use functional updater pattern to avoid stale closure on myProjects
      setCrRequests(all.filter((r: any) =>
        myProjects.some((p: any) => p.id === r.project_id) &&
  ...
  useEffect(() => {
    if (myProjects.length > 0) fetchCRRequests();
  }, [myProjects, fetchCRRequests]);

  // Fetch pm_review CRs for PM users
  const fetchPMCRRequests = useCallback(async () => {
    if (!isPM) return;
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL || ""}/api/client-requests/all`);
      if (!res.ok) return;
      const all: any[] = await res.json();
      setPmCrList(all.filter((r: any) =>
        myProjects.some((p: any) => p.id === r.project_id) && r.approval_stage === 'pm_review'
      ));
    } catch (e) { console.error(e); }
  }, [isPM, myProjects]);

  useEffect(() => {
    if (myProjects.length > 0) fetchPMCRRequests();
  }, [myProjects, fetchPMCRRequests]);

  const handlePmCrApprove = async () => {
    if (!pmCrModal) return;
    const isCRType = ['change_request', 'new_requirement'].includes(pmCrModal.request_type);
    if (isCRType && !pmCrForm.effort_man_days) { alert('Effort in man-days is required for this request type.'); return; }
    setPmCrSubmitting(true);
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL || ""}/api/projects/${pmCrModal.project_id}/client-requests/${pmCrModal.id}/pm-review`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'approve',
          pm_notes: pmCrForm.pm_notes,
          effort_man_days: pmCrForm.effort_man_days ? parseFloat(pmCrForm.effort_man_days) : undefined,
          effort_hours: pmCrForm.effort_hours ? parseFloat(pmCrForm.effort_hours) : undefined,
          pm_user_id: user?.id,
        }),
      });
      if (!res.ok) { const d = await res.json().catch(() => ({})); alert(d.error || 'Failed to submit PM review.'); return; }
      setPmCrModal(null);
      setPmCrForm({ pm_notes: '', effort_man_days: '', effort_hours: '' });
      fetchPMCRRequests();
    } finally { setPmCrSubmitting(false); }
  };

  const handlePmCrReject = async (req: any) => {
    if (!window.confirm('Reject this CR request?')) return;
    setPmCrSubmitting(true);
    try {
      await fetch(`${process.env.REACT_APP_API_URL || ""}/api/projects/${req.project_id}/client-requests/${req.id}/pm-review`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  const NAV_ITEMS = [
    ...
    // RFP Workspace temporarily disabled
    // { label: 'RFP Workspace', path: '/rfp', icon: (...) },
    {
      label: 'Analytics',
      path: '/analytics',
      icon: (
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
            <div className="px-6 py-4 bg-gradient-to-r from-indigo-600 to-violet-600 flex items-center justify-between">
              <div className="min-w-0">
                <p className="text-white font-bold text-sm">PM Review</p>
                <p className="text-white/70 text-xs mt-0.5 truncate max-w-xs">{pmCrModal.title}</p>
              </div>
              <button onClick={() => setPmCrModal(null)} className="text-white/70 hover:text-white text-xl leading-none flex-shrink-0">&times;</button>
            </div>

            <div className="px-6 py-5 space-y-4">
              {/* CR summary */}
              <div className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 space-y-1">
                <p className="text-xs font-semibold text-gray-700">{pmCrModal.title}</p>
                <p className="text-[10px] text-gray-400">{pmCrModal.request_type?.replace(/_/g,' ')} · #{pmCrModal.id} · {pmCrModal.project_name}</p>
                {pmCrModal.csm_notes && <p className="text-[10px] text-indigo-600 mt-1"><span className="font-semibold">CSM notes:</span> {pmCrModal.csm_notes}</p>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-gray-700 mb-1">
                    Effort (Man-days)
                    {['change_request','new_requirement'].includes(pmCrModal.request_type) && <span className="text-red-500 ml-0.5">*</span>}
                  </label>
                  <input type="number" min="0" step="0.5"
                    value={pmCrForm.effort_man_days}
                    onChange={e => setPmCrForm(f => ({ ...f, effort_man_days: e.target.value }))}
                    placeholder="e.g. 5"
                ...
                <div>
                  ...
                  <input type="number" min="0" step="0.5"
                    value={pmCrForm.effort_hours}
                    onChange={e => setPmCrForm(f => ({ ...f, effort_hours: e.target.value }))}
                    placeholder="e.g. 40"
                    className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-400" />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea rows={3}
                  value={pmCrForm.pm_notes}
                  onChange={e => setPmCrForm(f => ({ ...f, pm_notes: e.target.value }))}
                  placeholder="Assessment, conditions, follow-up…"
              ...
              <div className="flex gap-3 pt-1">
                <button onClick={handlePmCrApprove} disabled={pmCrSubmitting}
                  ...
                  {pmCrSubmitting ? 'Submitting…' : 'Approve — Forward to Sales'}
                </button>
                <button onClick={() => setPmCrModal(null)} disabled={pmCrSubmitting}
                  className="px-5 py-2.5 text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 disabled:opacity-40 rounded-xl transition-colors">
                  Cancel
                </button>
              </div>
            </div>
      ...
      <div className="flex h-screen overflow-hidden" style={{ background: 'var(--color-bg)' }}>
        ...
        <AppSidebar
          ...
          navItems={NAV_ITEMS.map(item => ({
            label: item.label,
            icon: item.icon,
            active: item.path === '/rfp'
              ? location.pathname.startsWith('/rfp')
              : location.pathname === item.path,
            onClick: () => { navigate(item.path); setSidebarOpen(false); },
          }))}
          userName={user?.name ?? ''}
          userRole={isCSM ? 'Customer Success Manager' : isProductManager ? 'Product Manager' : 'Project Manager'}
          userInitials={initials}
          onLogout={() => { logout(); navigate('/login'); }}
          onTourReplay={() => { setTourStep(0); setTourActive(true); }}
        />
        ...
        <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-hidden flex gap-0">
            ...
            <aside ref={rightPanelRef} className="hidden lg:block w-80 flex-shrink-0 border-l border-gray-100 bg-white overflow-y-auto px-4 py-5 space-y-3">
                ...
                return (
                  <div className="bg-white rounded-xl border border-gray-100 p-4">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <p className="text-[11px] font-bold text-gray-700 uppercase tracking-widest">Focus Today</p>
                    </div>
                    <div className="space-y-2">
                ...
                return (
                  <div className="bg-white rounded-xl border border-gray-100 p-4">
                    {crPending.length > 0 && (
                      <>
                        <div className="flex items-center justify-between mb-3">
                          <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">CR Requests</p>
                    ...
                    {actionItems.length > 0 && (
                      <>
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-2">Action Items</p>
                        <div className="space-y-2 mb-4">
              ...
              {/* ── CR Requests (PM only) ──────────────────────────────── */}
              {isPM && pmCrList.length > 0 && (
                <div className="bg-white rounded-xl border border-gray-100 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">CR Reviews</p>
                    <span className="text-[10px] font-bold bg-indigo-100 text-indigo-700 px-2 py-0.5 rounded-full">{pmCrList.length} pending</span>
                  </div>
                  <div className="space-y-2 mb-4">
                    {pmCrList.map((req: any) => (
                      <div key={req.id} className="p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
                        <div className="flex items-start justify-between gap-2 mb-1.5">
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-gray-800 truncate">{req.title}</p>
                            <p className="text-[10px] text-indigo-600 font-medium">{req.request_type?.replace(/_/g,' ')} · {req.priority}</p>
                            <p className="text-[10px] text-gray-400 truncate">{req.client_name} · {req.project_name}</p>
                          </div>
                        </div>
                        {req.csm_notes && (
                          <p className="text-[10px] text-gray-500 line-clamp-2 mb-2 italic">CSM: {req.csm_notes}</p>
                        )}
                        <p className="text-[10px] text-gray-500 line-clamp-2 mb-2">{req.description}</p>
                        <div className="flex gap-2">
                          <button
                            onClick={() => { setPmCrModal(req); setPmCrForm({ pm_notes: '', effort_man_days: '', effort_hours: '' }); }}
                            className="flex-1 py-1.5 text-[10px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">
                            Review
                          </button>
                          <button
                            onClick={() => handlePmCrReject(req)}
                            className="flex-1 py-1.5 text-[10px] font-bold text-red-700 bg-red-50 hover:bg-red-100 border border-red-200 rounded-lg transition-colors">
                            Reject
                          </button>
                        </div>
              ...
              {(() => {
                ...
                return (
                  <div className="bg-white rounded-xl border border-gray-100 p-4">
                    <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest mb-3">Action Required</p>
                    <div className="space-y-2">
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">My Tasks</p>
              ...
              <div className="bg-white rounded-xl border border-gray-100 p-4">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Milestones</p>
                </div>
                {(() => {
