...
const { setupSSORoutes }   = require('./modules/auth/ssoRoutes');
...
// const rfpRoutes            = require('./modules/rfp/rfpRoutes'); // RFP temporarily disabled
...
app.post('/api/users', async (req, res) => {
  try {
    ...
    const [id] = await db('users').insert({ name, email, role, department: department || null, password_hash, is_active: true, must_change_password: true });
    const user = await db('users').select('id', 'name', 'email', 'role', 'department', 'is_active', 'must_change_password').where({ id }).first();

    // Send welcome email with credentials
    const loginUrl = process.env.CLIENT_URL || 'http://localhost:3000';
    sendEmail({
      to: email,
      subject: '[Chaos Coordinator] Your account is ready',
      html: `<p>Hi ${name},</p>
             <p>Your Chaos Coordinator account has been created.</p>
             <p><strong>Role:</strong> ${role}<br>
             <strong>Email:</strong> ${email}<br>
             <strong>Temporary Password:</strong> ${plainPassword}</p>
             <p><strong>Login URL:</strong> <a href="${loginUrl}">${loginUrl}</a></p>
             <p style="color:#b91c1c;"><strong>IMPORTANT:</strong> For security, you will be prompted to change your password on first login.</p>
    ...
    db('activity_log').insert({
      project_id: null,
      action: 'user_created',
      details: JSON.stringify({ id: user.id, name: user.name, email: user.email, role: user.role }),
...
app.put('/api/users/:id', async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      project_id: null,
      action: 'user_updated',
      details: JSON.stringify({ id: req.params.id, changes: Object.keys(updates).filter(k => k !== 'updated_at') }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
app.delete('/api/projects/:projectId/discussions/:msgId', async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      ...
      details: JSON.stringify({ project_id: req.params.projectId, msg_id: req.params.msgId }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
app.post('/api/projects/:id/documents', upload.single('file'), async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      ...
      details: JSON.stringify({ project_id: pid, filename: req.file.originalname, category: category || 'General' }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
app.delete('/api/projects/:projectId/documents/:docId', async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      ...
      details: JSON.stringify({ project_id: req.params.projectId, doc_id: req.params.docId }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
app.post('/api/projects/:id/client-requests', async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      ...
      details: JSON.stringify({ id, project_id: pid, type: request_type, title: title.trim(), client_name: client_name || 'Client' }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
app.put('/api/projects/:id/client-requests/:reqId/csm-review', async (req, res) => {
  try {
    ...
    // mom_attendees arrives as a JSON-stringified array e.g. '["John","Jane"]' or '[]'
    let parsedAttendees = [];
    try { parsedAttendees = mom_attendees ? JSON.parse(mom_attendees) : []; } catch { parsedAttendees = []; }
    const hasAttendees = Array.isArray(parsedAttendees) && parsedAttendees.length > 0;
    if (action === 'approve' && !mom_file_path?.trim() && !hasAttendees) {
      return res.status(400).json({ error: 'A MoM summary or at least one attendee is required for CSM approval' });
    }
    ...
    await db('activity_log').insert({
      ...
      details: JSON.stringify({ req_id: reqId, action, project_id: pid }),
      created_at: now,
    }).catch(() => {});
...
app.put('/api/projects/:id/client-requests/:reqId/pm-review', async (req, res) => {
  try {
    ...
    await db('activity_log').insert({
      ...
      details: JSON.stringify({ req_id: reqId, action, effort_man_days, project_id: pid }),
      created_at: now,
    }).catch(() => {});
...
app.put('/api/projects/:id/client-requests/:reqId/sales-review', async (req, res) => {
  try {
    ...
    await db('activity_log').insert({
      ...
      details: JSON.stringify({ req_id: reqId, action, billing_type, project_id: pid }),
      created_at: now,
    }).catch(() => {});
...
app.put('/api/projects/:id/client-requests/:reqId/admin-review', async (req, res) => {
  try {
    const pid = parseInt(req.params.id);
    const reqId = parseInt(req.params.reqId);
    const { action, admin_notes, admin_user_id, due_days } = req.body;
    if (!action || !['approve', 'reject'].includes(action)) return res.status(400).json({ error: 'action must be approve or reject' });
    ...
    const updates = {
      ...
      ...(action === 'approve'
        ? { approval_stage: 'approved', status: 'approved', is_team_visible: true, approved_at: now,
            due_date: new Date(Date.now() + (parseInt(due_days) || 3) * 24 * 60 * 60 * 1000).toISOString() }
    ...
    await db('activity_log').insert({
      ...
      details: JSON.stringify({ req_id: reqId, action, admin_notes, project_id: pid }),
      created_at: now,
    }).catch(() => {});
...
app.put('/api/users/:id/assign-projects', async (req, res) => {
  try {
    ...
    db('activity_log').insert({
      project_id: null,
      action: 'client_projects_assigned',
      details: JSON.stringify({ user_id: req.params.id, project_ids }),
      created_at: new Date().toISOString(),
    }).catch(() => {});
...
// PUT /api/users/:id/change-password — user changes their own password on first login
app.put('/api/users/:id/change-password', async (req, res) => {
  try {
    ...
    await db('users').where({ id: req.params.id }).update({ password_hash: hash, must_change_password: false, updated_at: new Date().toISOString() });
    // Log to activity_log
    await db('activity_log').insert({ action: 'user_password_changed', details: JSON.stringify({ user_id: req.params.id }), created_at: new Date().toISOString() });
    res.json({ success: true });
  } catch (e) { console.error(e); res.status(500).json({ error: 'Failed to change password' }); }
});
...
// app.use('/api/rfp',           rfpRoutes); // RFP temporarily disabled
